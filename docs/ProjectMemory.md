...
- 2026-09-12 — First manual testing round (`TestingFeedback.md`): 2 findings, both triaged. Sample-bootcamp "already exists" = data state, not code (BUILD-11) — AJ's series reset left a stale record behind. Attendee line getting blank Bootcamp No. = real bug, fixed via a defensive `OnNewRecord` on `ocpfAttendeeSubform` (BUILD-12). 20 files, 0/0. Package still not rebuilt since BUILD-09/10 (still `0.0.1.0` in `out/`).
