...
- 2026-09-12 — First manual testing round (`TestingFeedback.md`): 2 findings. Attendee line getting blank Bootcamp No. = real bug, fixed via a defensive `OnNewRecord` on `ocpfAttendeeSubform` (BUILD-12). Sample-bootcamp "already exists" was first misdiagnosed as stale data (BUILD-11) — AJ disproved that with an empty table + a brand-new series reproducing it — corrected diagnosis: both sample inserts computed the same number, second collided, error rolled back both (looks like empty table + "already exists" together). Fixed with self-healing number assignment in `ocpfBootcampRegMgt` (BUILD-13). 20 files, 0/0. Package still not rebuilt since BUILD-09/10 (still `0.0.1.0` in `out/`).
