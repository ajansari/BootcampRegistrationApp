...
| F-3 | Show **Seats Remaining** on a bootcamp = Max Seats − current attendee count, maintained automatically and never entered by a user. Also expose a live **Registered Attendees** count. **When `Max Seats = 0` (no cap, F-10), Seats Remaining always shows 0** rather than a negative number (ChangeLog STEP09-02, BP-7). |
...
| F-8 | Capture on an attendee: Paid (yes/no), Payment Date, Amount Paid. Amount Paid defaults from the bootcamp Price **the moment a bootcamp is selected**, stays editable, and is never auto-overwritten afterward — including an explicit **0** entered for a comped/free registration (ChangeLog STEP09-02, BP-1: the original design re-seeded on every insert whenever the field read as 0, which couldn't distinguish "not yet supplied" from "deliberately zero," and silently re-billed a comp). |
...
| D-6 | API pages: `ODataKeyFields = SystemId`; exactly one of `DelayedInsert = true` (editable) or `Editable = false` (read-only) per §7 mutability. **As shipped:** `APIPublisher = 'onlyCopilotFans'`, `APIGroup = 'ocpfBootcampRegistration'`, `APIVersion = 'v1.0'` — camelCase, no `<prefix>_` separator, an AA0101 divergence from the Standards §1.3 literal example (ChangeLog BUILD-06). *(Corrected at Step 10 — this row still showed the pre-BUILD literal values `'OnlyCopilotFans'` / `'ocpf_bootcampRegistration'`, which were never actually built; `ProjectParameters.md` §1.3 was already corrected for the same drift at Step 08's G-17, but this FRD row was missed then.)* |
| D-7 | No dead code: no empty triggers, no commented-out fields, no `// TODO`. |
| D-8 | Seats Remaining is a **non-editable field maintained automatically** by `ocpfAttendee` table event subscribers (OnAfterInsert/Modify/Delete/Rename) and by the Bootcamp `Max Seats` `OnValidate`; users and API callers never write it. A companion `FlowField` **`Registered Attendees`** gives the live count. Clamped to 0 when Max Seats = 0 (F-3). *(Original D-8 required a pure FlowField; AL `FlowField` `CalcFormula` cannot express "Max Seats minus a count", so a subscriber-maintained stored field is used — see ChangeLog Issue DESIGN-02. The modify subscriber's before/after comparison was corrected at Step 09, STEP09-02 BP-2 — `xRec` is not a reliable before-image on a code- or API-driven `Modify()`, only on a page-driven one.)* |
| D-9 | Amount Paid is a **stored** field seeded **exactly once, from `"Bootcamp No."`'s `OnValidate`** (not re-seeded on every insert) from the bootcamp Price; the seeding logic must not overwrite a value the user (or an API caller) has already supplied, including an explicit 0. *(Corrected at Step 09, STEP09-02 BP-1 — the original "seed on insert if Amount Paid = 0" guard couldn't tell a not-yet-supplied value from a deliberate zero.)* |
...
| Set | Grants |
|---|---|
| `OCPF - Bootcamp Read` | `tabledata` read on Bootcamp, Attendee, Bootcamp Registration Setup, **plus explicit `page … = X` execution grants on all 8 of this extension's own pages** — see TDD §6.16. *(Corrected at Step 09, `CodeReview.md` SC-1: the original design deferred these grants pending a live test that never actually ran, relying instead on BC's inherent-permission model; Step 09 added them defensively per Standards §7.3's literal requirement rather than continue treating the assumption as verified. Still to confirm live with a non-SUPER test user — a named Step 12 test case, not closed by this correction alone.)* |
